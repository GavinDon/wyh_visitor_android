...
import com.stxx.wyhvisitorandroid.bean.HotDotResp
...
class ScenicVm : MVVMBaseViewModel() {
    ...
    private val hotDotLiveData = singLiveData<List<HotDotResp>>()
     ...
     * 获取热力图
     */
    fun getHotMapDot(): singLiveData<List<HotDotResp>> {
        model.fetchHotDot({
            hotDotLiveData.value = Resource.create(it)
        }, {
            hotDotLiveData.value = Resource.create(it)
        })
        return hotDotLiveData
    }
...
class ScenicRepositories(private val mComDis: CompositeDisposable) : MVVMBaseModel() {
    ...
    fun fetchHotDot(
        ...
        val t = object : TypeToken<BR<List<HotDotResp>>>() {}.type
        mComDis.add(
            http!!.get(ApiService.HOT_DOT)
                .parse2<BR<List<ScenicCommentResp>>>(t, {
                    onSuccessT.invoke(it)
                }, {
                    onFailed.invoke(it)
